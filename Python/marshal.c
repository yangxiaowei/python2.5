...
static void
w_object(PyObject *v, WFILE *p)//对象v写入p中的[strings]字典
{
	Py_ssize_t i, n;
...
#endif
	else if (PyString_Check(v))  /*检查到v是string类型的数据*/ 
    {
		if (p->strings && PyString_CHECK_INTERNED(v))  /*检查到string为INTERNED的且字典不为空*/ 
        {
			PyObject *o = PyDict_GetItem(p->strings, v); /*取出对应的interned对象的序号*/ 
			if (o) 
            {
				long w = PyInt_AsLong(o);//intobject->long
				w_byte(TYPE_STRINGREF, p);
                //类型也不是简单的TYPE_STRING,更不是首次interned的TYPE_INTERNED,这样告诉解析器：
                //我后面那个数字是一个引用，引用第w个字符
				w_long(w, p);
				goto exit;
			}
			else 
