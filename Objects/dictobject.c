...
static PyDictObject *free_dicts[MAXFREEDICTS];//有一个80个字典大小的池子供大家使用
static int num_free_dicts = 0;

PyObject *
PyDict_New(void)
{
	register dictobject *mp;//临时变量用reigister，可以加快运行速度
	if (dummy == NULL) //为什么不设置静态变量，而每次初始化一个dict都要进行dummy检查呢？这样不是影响效率吗？
    { /* Auto-initialize dummy */
		dummy = PyString_FromString("<dummy key>");//看见dummy的key是什么了吧？
		if (dummy == NULL)
			return NULL;
#ifdef SHOW_CONVERSION_COUNTS
    ...
    //dict缓冲池,可见这种动态语言中池是经常被使用的
	if (num_free_dicts)  /*如果池子中有，则从池子中取*/ 
    {
		mp = free_dicts[--num_free_dicts];
		assert (mp != NULL);//值检查
		assert (mp->ob_type == &PyDict_Type);//类型检查
		_Py_NewReference((PyObject *)mp);
		if (mp->ma_fill)//池子里dict是不是之前别人遗弃的 
        {
			EMPTY_TO_MINSIZE(mp);//初始化ma_fill，ma_used,清空ma_smalltable，
                                //并执行INIT_NONZERO_DICT_SLOTS
		} 
        else 
        {
    ...
    else /*如果池子里没有，则只能从堆中生成一个崭新的dict*/ 
    {
	...
	mp->ma_lookup = lookdict_string;//hash策略的选择
#ifdef SHOW_CONVERSION_COUNTS
	++created;
#endif
	_PyObject_GC_TRACK(mp);//垃圾回收监控初始化操作
	return (PyObject *)mp;
}
...
lookdict(dictobject *mp, PyObject *key, register long hash)
{
	...
	i = (size_t)hash & mask;
	ep = &ep0[i];//不用进行越界检查，当mp->ma_table指向的是small_table呢?
	...
	else 
    {
		if (ep->me_hash == hash) 
        {
			startkey = ep->me_key;
			Py_INCREF(startkey);
            /* Return -1 if error; 1 if v op w; 0 if not (v op w),here op is Py_EQ. */
			cmp = PyObject_RichCompareBool(startkey, key, Py_EQ);
			Py_DECREF(startkey);
			if (cmp < 0)//比较失败
				return NULL;
			if (ep0 == mp->ma_table && ep->me_key == startkey) //检查比较是否导致字典内部数据错误
            {
				if (cmp > 0)//如果startkey与key相同，即指针相同，则说明查找成功
					return ep;
			}
			else {
    ...
    /*1、比较失败;
     *2、开放地址法中初次查找成功;
     *3、hash对应的ep位置并没有使用过（包括key自己也没有使用过个位置*/
    /*并且有以下两种可能，查找到的ep要么处于dumy状态，要么处于已经被使用*/

	/* In the loop, me_key == dummy is by far (factor of 100s) the
	   least likely outcome, so test for that last. */
	for (perturb = hash; ; perturb >>= PERTURB_SHIFT) {
		i = (i << 2) + i + perturb + 1;//这是二次探测的hash函数
		ep = &ep0[i & mask];//取模，获得entry指针
		if (ep->me_key == NULL)
			return freeslot == NULL ? ep : freeslot;//如果freelot不为空，则返回freelot，即处于dummy态的entry，优先使用前面的可使用entry
		if (ep->me_key == key)//查找自身，查找成功,指针层次上的相同
			return ep;
		if (ep->me_hash == hash && ep->me_key != dummy) //hash值相同，并且不处于dummy态
        {
			startkey = ep->me_key;
			Py_INCREF(startkey);
			cmp = PyObject_RichCompareBool(startkey, key, Py_EQ);//指针指向的内容是否相同
			Py_DECREF(startkey);
			if (cmp < 0)
				return NULL;
			...
			freeslot = ep;//如果freeslot是null，则记住dummy态的entry，以达到先使用处于dummy态的entry的目的
	}
...
lookdict_string(dictobject *mp, PyObject *key, register long hash)
{
	...
	i = hash & mask;//截取字典长度所对应的有效部分
	ep = &ep0[i];
	if (ep->me_key == NULL || ep->me_key == key)//如果此key所映射的entry没使用，或此key本来就是此字典中的key
		return ep;
	if (ep->me_key == dummy)//此key第一次映射的位置已经被人使用并抛弃过
		freeslot = ep;
	else//还有一种情况，就是相同内容的串，可能由不同的对象来容纳，这与intern机制不符啊？ 
    {
		if (ep->me_hash == hash && _PyString_Eq(ep->me_key, key))//hash值相同，说明内容可能相同，因此再对串进行比较
			return ep;
		freeslot = NULL;
	}
...
{
	...
	if (ep->me_value != NULL) //查找成功
    {
		...
		if (ep->me_key == NULL)//处于未使用状态，更新曾使用标志
			mp->ma_fill++;
		else //处于dummy态
        {
...
dictresize(dictobject *mp, Py_ssize_t minused)
{
	...
	is_oldtable_malloced = oldtable != mp->ma_smalltable; /*dict之前有没有使用完smalltable而开辟新空间*/ 

	if (newsize == PyDict_MINSIZE) 
    {
		/* A large table is shrinking, or we can't get any smaller. */
		newtable = mp->ma_smalltable;
		if (newtable == oldtable)  /*如果没有开辟新的空间*/ 
        {
			if (mp->ma_fill == mp->ma_used) //没有dummy态的entry，也就没有必要进行空间的收缩扩张了
            {
			...
			/* We're not going to resize it, but rebuild the
			   ...
			   as lookdict needs at least one virgin slot to :lookdict需要至少一块处女地来确保查找失败
...
PyDict_SetItem(register PyObject *op, PyObject *key, PyObject *value)
{
	...
	if (!(mp->ma_used > n_used && mp->ma_fill*3 >= (mp->ma_mask+1)*2))
		return 0;
    /*超过50000,则成双倍增长，否则成4倍增长,计算基础可是ma_used，即不包括dummy状态的，只有active状态*/
