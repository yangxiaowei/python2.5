...
list_resize(PyListObject *self, Py_ssize_t newsize)
{
	...
	*/
    //当新的空间大小 大于最大空间的一半，但并不大于最大空间时
    //仅仅改变一下大小、即可,然后在insl往后挪动元素即可
	if (allocated >= newsize && newsize >= (allocated >> 1)) 
    {
        /*printf("list_resize debug\tallocated:%d\tnewsize:%d\tallocated/2:%d\n",allocated,newsize,allocated>>1);*/
	 ...
	 */
    //现面的这个new_allocated的值为即将要额外扩充的大小，还记得预申请吗？
	new_allocated = (newsize >> 3) + (newsize < 9 ? 3 : 6);
                    // 新大小的1/8 + 3（新大小小于9）否则为6
    /*printf("allocated-%d new_allocated-:%d ",allocated, new_allocated);*/
    /*int temp = new_allocated;*/
    ...
    /*printf("newsize-%d new_allocated-:%d\n",newsize,new_allocated);*/
    //现在就可以明白预申请的机制实现了，即如果在insl中调用list_resize时，可以看到这个
    //newsize=n+1，即多一个元素，但到这里面来，情况就变了。实现要扩充的大小变成：
    //new_allocated+newsize，也就是 n+1 + new_allocated
    ...
    /*{                                                                                                        */
    /*    printf("list_resize shrink");                                                                        */
    /*}                                                                                                        */
    /*else                                                                                                     */
    /*{                                                                                                        */
    /*    printf("list_resize expand");                                                                        */
    /*}                                                                                                        */
    /*printf("\tob_size:%d\told_new_allocated:%d\tnew_allocated:%d\tallocated:%d\tnewsize:%d\tnewsize>>3:%d\n",*/
    /*        self->ob_size,temp,new_allocated,allocated,newsize,newsize>>3);                                  */
...
#define MAXFREELISTS 80 //list池的list数量
static PyListObject *free_lists[MAXFREELISTS];
//这就是list的池,可是池中存放的是指针呢，那从池子中取出来，不是还要malloc一个list对象吗？
//哦，可能又是’用时生成‘的思想
//这次猜的有点小错，python用了一种，丢时进池的思想，算是用时生成吧。不过很迂回。
...
PyList_New(Py_ssize_t size)
{
	...
	nbytes = size * sizeof(PyObject *);/*看见没，计算空间大小时，其单元也是PyObject指针*/
	/* Check for overflow without an actual overflow,
	 *  which can cause compiler to optimise out */
	if (size > PY_SIZE_MAX / sizeof(PyObject *))
        /*估计PY_SIZE_MAX存放的是总字节大小*/ 
    ...
    {
		op = PyObject_GC_New(PyListObject, &PyList_Type);//池子里没有，就重新生成一个呗
		if (op == NULL)
			return NULL;
	}
    //好了，list对象生成完毕，下面轮到生成存放元素指针的空间了
	if (size <= 0)
		op->ob_item = NULL;
	else 
    {
               ...
               register PyObject *newitem)
{
    ...
    //可能此槽处之前有一个对象,即用newitem替换下之前的对象
	olditem = *p;
	*p = newitem;
    //引用数减1
	Py_XDECREF(olditem);
...
ins1(PyListObject *self, Py_ssize_t where, PyObject *v)
{
    ...
    //扩展list空间到n+1,通过list_resize我们可以发现，
    //并不是简单地增长一个单位，非线性的增长，
    //更加奇怪的是陈儒说list可能会因此收缩,可是并没有看到，难道是在删除的时候来收缩
	...
	if (where > n)//默认插入尾部，而不是隔空插入,
		where = n;//就是当插入位置大于当前最后一个元素的索引值,那么默认为最大索引值+1
	items = self->ob_item;
    //搬移
	for (i = n; --i >= where; )
		items[i+1] = items[i];
...
reverse_slice(PyObject **lo, PyObject **hi)
{
	...
	--hi;
    //从最开始一个元素，与最后一个元素开始，两两交换
    //--->...................<---
    //|________exchange_________|
	while (lo < hi) {
		PyObject *t = *lo;
		*lo = *hi;
		*hi = t;
