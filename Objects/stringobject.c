...
PyString_FromString(const char *str)
{
    ...
    /*长度为0,且nullstring不是NULL，则返回nullstring的指针,又是利用策略*/
	if (size == 0 && (op = nullstring) != NULL) {
#ifdef COUNT_ALLOCS
		null_strings++;
#endif
    ...
    /*如果是单个字符，恐怕python维护一张可打印字符表，然后复用之*/ 
    /*号称python尊老爱幼，呵呵*/
    /*由于python使用了‘用时初始化’的思路，所以可能为NULL然后由后面*/
    /*对单字符进行interned操作*/
    ...
    /*很纳闷，无论后面是不是使用intern机制来共享已有的字符串*/
    /*都要malloc一个PyStringObject对象出来                  */
    /*这是因为PyString_InternInPlace需要一个PyStringObject对象*/
    /*看来，intern机制并不是为了提高效率，而是为了节约内存*/
	...
	if (op == NULL)
		return PyErr_NoMemory();
	PyObject_INIT_VAR(op, &PyString_Type, size);
	op->ob_shash = -1; /*哈希值先别算，万一后面可以使用intern机制呢,那也得哈希值呢*/ 
	op->ob_sstate = SSTATE_NOT_INTERNED;/*初始化为未intern的字符串，先破再有立吗，还记得败者树的初始化吗？*/
	Py_MEMCPY(op->ob_sval, str, size+1);

	/* share short strings */
    /*python的intern机制,但好像这里只处理了长度为0和1的字符对象*/
    if (size == 0) {
		PyObject *t = (PyObject *)op;
		PyString_InternInPlace(&t);
		op = (PyStringObject *)t;
...
string_dealloc(PyObject *op)
{
	switch (PyString_CHECK_INTERNED(op)) 
    {
        ...
        /*靠，一旦发现是SSTATE_INTERNED_IMMORTAL,不准删除*/
...
_PyString_Eq(PyObject *o1, PyObject *o2)
{
	...
	PyStringObject *b = (PyStringObject*) o2;
        return a->ob_size == b->ob_size//串长度是否相等
          && *a->ob_sval == *b->ob_sval//串内容
          && memcmp(a->ob_sval, b->ob_sval, a->ob_size) == 0;//还得对指定大小(ob_size)的内存内容还得比较
          //如果a,b长度都是5 内容都abc。两者也不相同，所以还得经过最后一个测试来确定两者是否真的内容相同
}
...
string_hash(PyStringObject *a)
{
	...
	if (a->ob_shash != -1) /*如果之前计算过hash值，肯定会保存在ob_shash中，立即返回该值即可*/ 
		return a->ob_shash;

	len = a->ob_size;
	p = (unsigned char *) a->ob_sval; /*强制类型转化为指针类型，下面开始操作这个字符，计算出hash值*/ 
	x = *p << 7;
	while (--len >= 0)
		x = (1000003*x) ^ *p++;
	x ^= a->ob_size;
	if (x == -1)
		x = -2;
	a->ob_shash = x;
	return x; /*返回的是一个long类型的值:32位:42亿种可能，觉得也有可能出现重复值啊*/ 
}
...
string_join(PyStringObject *self, PyObject *orig)
{
    ...
    /*若sequence长度为空，则不管sep是什么，都返回空*/
	if (seqlen == 0) {
		Py_DECREF(seq);
		return PyString_FromString("");
	}
    /*若sequence长度为1,则不管sep是什么，都返回sequence的单个元素的字符串形式*/
	if (seqlen == 1) 
    {
		item = PySequence_Fast_GET_ITEM(seq, 0);
		if (PyString_CheckExact(item) || PyUnicode_CheckExact(item)) {
			Py_INCREF(item);//item增加引用计数
			Py_DECREF(seq);//seq减少引用计数
			return item;
		}
	}
    ...
    /*然后将所有的串进行统一的拷贝，这当然要比虽然内层循环都是n次，但使用‘+’导致n次string对象的生成*/
	for (i = 0; i < seqlen; ++i) {
		size_t n;
		item = PySequence_Fast_GET_ITEM(seq, i);
...
PyString_InternInPlace(PyObject **p)
{
	...
	if (interned == NULL) 
    {
        /*这是一个全局的字典，利用string的hash值与其value进行一一映射*/
		interned = PyDict_New();
		if (interned == NULL) 
        {
    ...
    /*这儿再次说明了，前面那个PyStringObject即便后面使用了intern机制也需要创建，*/
    /*因为好多接口函数都需要一个PyStringObject对象作为参数进行统一的处理         */
	t = PyDict_GetItem(interned, (PyObject *)s);
	if (t) {
        /*经过检测得知，已经有一个长的一模一样的string存在interned中了，所以将之前那个销毁*/
        /*，并将interned中的那人引用计数加一，以示其新增一个‘副本’存在                    */
		Py_INCREF(t);
		Py_DECREF(*p);
		*p = t;
		return;
    ...
    /*看样子，之前没有人生成同样的字符串，那么只好开天下之先了，在interned中生成一个*/
	...
	s->ob_refcnt -= 2;
    /*下面讲一下，这里为什么减2,我们注意到上面的PyDict_SetItem函数的参数有两个s，其实一个是作为key*/
    /*一个作为value。根据统一化标准，显然s被interned这个字典进行了两次引用。但这并不是我们需要的引用。*/
    /*如果不这样，设想一下，如果用户真实的串全部销毁时，那么s的引用应该是0,但很遗憾，interned字典还*/
    /*作了2的若干倍数次引用*/
    /*从中可以看出，python的基本数据类型，不仅属性用户，python自己也在使用，而且执行相同的标准*/

    /*interned里的字符串有两种状态:SSTATE_INTERNED_IMMORTAL与SSTATE_INTERNED_MORTAL状态*/
    /*前者表明，这个串将与python虚拟机同年同月同日亡，而后者则只要当没有字符串再引用它了，就over了*/
    /*这可以在string_dealloc中看到*/
...
PyString_InternFromString(const char *cp)
{
    ...
    /*printf("come in PyString_InternInPlace\n");            */
    /*printf("address:\t%p\n",s);                            */
    /*printf("string is %s\n",((PyStringObject*)s)->ob_sval);*/
	return s;
}
