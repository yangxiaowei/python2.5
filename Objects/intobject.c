...
/*函数外部的static声明，表示此变量为 static with internal*/
/*即生命周期为全局，但访问域为此文件内部，外部不可external*/
static PyIntBlock *block_list = NULL;
static PyIntObject *free_list = NULL;

/*申请一个block，来存放’大‘整数，并将其结构化成PyIntObject单链*/ 
static PyIntObject *
fill_free_list(void)
{
	PyIntObject *p, *q;
    ...
    /*PyMem_MALLOC 在pymem.h是一个包装了malloc的宏，只不过处理了传入malloc的
     * 参数为0或负数的异常情况*/
	if (p == NULL)
		return (PyIntObject *) PyErr_NoMemory();
    /*由block_list将所有的Block串成单链*/
	...
	q = p + N_INTOBJECTS; /*指针式的增加哦，可不是直接加上一个N_INTOBJECTS*/ 
    /*从最后一个整数q对象开始，每个的ob_type存储其前一个整数对象的指针*/
	while (--q > p)
		q->ob_type = (struct _typeobject *)(q-1);
	q->ob_type = NULL; /*第一个是单链表的最后一个结点，其‘next’指针竟然要重新置成空了*/ 
	return p + N_INTOBJECTS - 1;
}

/*小整数池中最大的正数为257;最小的负数为 -5*/ 
...
PyInt_FromLong(long ival)
{
	...
	/* Inline PyObject_New */
	v = free_list;
	free_list = (PyIntObject *)v->ob_type; /*看样子，整数池中的每个整数对象在没有使用前，其ob_type属性成了'next'指针*/ 
	PyObject_INIT(v, &PyInt_Type);         /*58 行 验证这个假想*/
	v->ob_ival = ival;
	return (PyObject *) v;
}
...
int_dealloc(PyIntObject *v)
{
	if (PyInt_CheckExact(v)) {
		...
		free_list = v;
        /*看见没，释放的对象又回到被free_list索引的单链表中
         * 也就是对于free_list而言，每个PyIntObject对象是否在同一个block中，它并不关心*/ 
	}
	else
     ...
     /* flags -- not used but required by interface */
{
    ...
    /*for(i=0;i<10;i++)                           */
    /*    printf("%ld\t",small_ints[i]->ob_ival); */
    /*printf("\n");                               */
    /*for(i=0;i<10;i++)                           */
    /*    printf("%d\t",small_ints[i]->ob_refcnt);*/
    /*printf("\n");                               */
    ...
    /*printf("There are %d int-block used....\n",i);*/
	return 0;
}
...
_PyInt_Init(void)
{
	...
	int ival;
#if NSMALLNEGINTS + NSMALLPOSINTS > 0
    /*小整数池里放的小整数,也是从’大‘整数池中取出来初始化成小整数的*/
    for (ival = -NSMALLNEGINTS; ival < NSMALLPOSINTS; ival++)
    {
        /*只要free_list牵引的链表里有剩余 或能申请到可以使用的PyIntObject的池*/ 
        if (!free_list && (free_list = fill_free_list()) == NULL)
            return 0;
        /* PyObject_New is inlined */
        v = free_list;
        free_list = (PyIntObject *)v->ob_type;
        PyObject_INIT(v, &PyInt_Type); /*object.c中可以看到这个是PyObject_INIT是完成所有对象初始化的入口，引用数、类型...初始化*/ 
        v->ob_ival = ival;
        small_ints[ival + NSMALLNEGINTS] = v; /*赋值给小整数了!!!*/ 
    }
#endif
	return 1;
}
